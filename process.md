...
## 프로그램 개요
문자열 치환 도구는 iflist03a.py에서 생성된 엑셀 파일을 기반으로 파일 복사 작업을 자동화하는 프로그램입니다. 주요 기능은 엑셀 파일에서 파일 복사 정보를 읽어 YAML 파일로 변환하고, 실제 파일 복사 작업을 수행하는 것입니다.

## 주요 기능

### 1. YAML 생성 (엑셀 -> YAML)
- iflist03a.py에서 생성된 엑셀 파일을 읽어 YAML 파일로 변환
- 일반행과 매칭행이 2행씩 쌍을 이루는 구조 처리
- 4가지 파일 타입에 대한 복사 정보 생성:
  1. 송신파일경로
  2. 수신파일경로
  3. 송신스키마파일명
  4. 수신스키마파일명
- 각 파일 타입별 생성 여부는 다음 컬럼의 값으로 결정:
  - '송신파일생성여부'
  - '수신파일생성여부'
  - '송신스키마파일생성여부'
  - '수신스키마파일생성여부'
- 생성 여부 값이 1.0인 경우에만 해당 파일 타입의 복사 정보 생성
...
### 3. 현재 구현 상태
- [x] pandas를 사용한 엑셀 파일 읽기
- [x] 일반행/매칭행 쌍 처리
- [x] 파일 생성 여부 확인 로직
- [x] YAML 구조 생성 및 디버그 출력
- [ ] 실제 YAML 파일 생성
- [ ] 파일 복사 기능 구현
- [ ] 로그 및 요약 파일 생성
...
### 1. 데이터 처리
- pandas를 사용한 효율적인 엑셀 파일 읽기
- float 타입의 숫자 값 처리 (1.0)
- NaN 값 안전 처리
